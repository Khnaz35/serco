...
class ControllerTestimonialTestimonial extends Controller
{
    ...
    public function index() {
        ...
        $data['review'] = $this->url->link($this->modulePath . '/review', '', true);
        $data['write'] = $this->url->link($this->modulePath . '/write', '', true);
    ...
    public function review() {
        ...
        $results = $this->{$this->moduleModel}->getReviews(($page - 1) * $reviews_limit, $reviews_limit);
        ...
        $pagination->limit = $reviews_limit;
        $pagination->url = 'index.php?route=' . $this->modulePath . '/review&page={page}';

        $data['pagination'] = $pagination->render();

        $data['results'] = sprintf($this->language->get('text_pagination'), ($review_total) ? (($page - 1) * $reviews_limit) + 1 : 0, ((($page - 1) * $reviews_limit) > ($review_total - $reviews_limit)) ? $review_total : ((($page - 1) * $reviews_limit) + $reviews_limit), $review_total, ceil($review_total / $reviews_limit));
    ...
    public function write() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ((utf8_strlen($this->request->post['text']) < 1) || (utf8_strlen($this->request->post['text']) > 3000)) {
