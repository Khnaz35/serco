...
class ControllerStartupSeoPro extends Controller {
	...
	public function index() {
		...
		if (!isset($this->request->get['_route_'])) {
			...
			if (isset($this->request->get['product_id'])) {
				...
				if (!isset($this->request->get['newsblog_path'])) {
					$newsblog_path = $this->getPathByArticle($this->request->get['newsblog_article_id']);
					if ($newsblog_path) $this->request->get['newsblog_path'] = $newsblog_path;
				}
		...
		switch ($route) {
			...
			case 'newsblog/article':
				if (isset($data['newsblog_article_id'])) {
					...
					$data = array();
					if ($this->config->get('config_seo_url_include_path')) {
						$data['newsblog_path'] = $this->getPathByArticle($tmp['newsblog_article_id']);
			...
			case 'newsblog/category':
				if (isset($data['newsblog_path'])) {
					...
					$category = end($category);
					$data['newsblog_path'] = $this->getPathByArticleCategory($category);
					if (!$data['newsblog_path']) return $link;
				}
				break;
		...
		foreach ($data as $key => $value) {
			switch ($key) {
				...
				case 'newsblog_category_id':
					...
					break;
				case 'newsblog_path':
					$categories2 = explode('_', $value);
					foreach ($categories2 as $category2) {
						$queries[] = 'newsblog_category_id=' . $category2;
					}
					unset($data[$key]);
					break;
	...
	private function getPathByArticle($article_id) {
		...
		if (!isset($path[$article_id])) {
			...
			$path[$article_id] = $this->getPathByArticleCategory($query->num_rows ? (int)$query->row['category_id'] : 0);
	...
	private function getPathByArticleCategory($category_id) {
		...
		static $path = null;
		if (!isset($path)) {
			$path = $this->cache->get('newscategory.seopath');
			if (!isset($path)) $path = array();
		}
		...
		if (!isset($path[$category_id])) {
			...
			$this->cache->set('newscategory.seopath', $path);
		}
