<?php
// MY.SMSCAB.RU API (my.smscab.ru) версия 3.4 (05.03.2015)

define("SMSC_LOGIN", $this->config->get('config_login_send_sms_fastorder'));			// логин клиента
define("SMSC_PASSWORD", $this->config->get('config_pass_send_sms_fastorder'));	// пароль или MD5-хеш пароля в нижнем регистре
...
define("SMTP_FROM", "api@my.smscab.ru");     // e-mail адрес отправителя
...
function send_sms($phones, $message, $translit = 1, $time = 0, $id = 0, $format = 0, $sender = false, $query = "", $files = array())
{
...
{
	$url = (SMSC_HTTPS ? "https" : "http")."://my.smscab.ru/sys/$cmd.php?login=".urlencode(SMSC_LOGIN)."&psw=".urlencode(SMSC_PASSWORD)."&fmt=1&charset=".SMSC_CHARSET."&".$arg;
