...
class ModelCatalogProduct extends Model {
	...
	public function getProductColors($product_id) {
		$sql = "SELECT cs.product_id, cs.color_name, cs.color_image FROM " . DB_PREFIX . "color_set cs LEFT JOIN " . DB_PREFIX . "color c ON (cs.color_set_id = c.color_set_id) WHERE c.status = '1' AND cs.color_set_id IN (SELECT cs.color_set_id FROM " . DB_PREFIX . "color_set cs WHERE cs.product_id = '" . (int)$product_id . "') AND cs.product_id IN (SELECT p.product_id FROM " . DB_PREFIX . "product p WHERE p.product_id = cs.product_id AND p.status = '1' AND p.date_available <= NOW()) AND cs.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY cs.sort_order ASC";
	...
	public function getProductRoznPrice($product_id) {

		$customer_group_id = 1;

		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, (SELECT md.name FROM " . DB_PREFIX . "manufacturer_description md WHERE md.manufacturer_id = p.manufacturer_id AND md.language_id = '" . (int)$this->config->get('config_language_id') . "') AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$customer_group_id . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
	...
	public function getProductMOptPrice($product_id) {
...
// ExtendedSearch END
	...
	public function getTotalProductSpecials() {
		...
		//$query = $this->db->query("SELECT rd.* FROM " . DB_PREFIX . "product_recurring pr JOIN " . DB_PREFIX . "recurring_description rd ON (rd.language_id = " . (int)$this->config->get('config_language_id') . " AND rd.recurring_id = pr.recurring_id) JOIN " . DB_PREFIX . "recurring r ON r.recurring_id = rd.recurring_id WHERE pr.product_id = " . (int)$product_id . " AND status = '1' AND pr.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' ORDER BY sort_order ASC");
                ...
                $query = $this->db->query("SELECT ovd.name, po.product_id,ov.image as color_image,'1' as selected  from oc_product_option po JOIN oc_product_option_value pov ON (po.product_option_id = pov.product_option_id) 
JOIN oc_option_value ov  ON (ov.option_value_id = pov.option_value_id)
left JOIN oc_option_value_description ovd  ON (ov.option_value_id = ovd.option_value_id)
 where  pov.option_id='13' and po.product_id='" . (int)$product_id . "' ORDER BY ov.sort_order");
		...
		return $query->rows;
             ...
             public function getRelativeProductsColor($product_id) {
		//$query = $this->db->query("SELECT rd.* FROM " . DB_PREFIX . "product_recurring pr JOIN " . DB_PREFIX . "recurring_description rd ON (rd.language_id = " . (int)$this->config->get('config_language_id') . " AND rd.recurring_id = pr.recurring_id) JOIN " . DB_PREFIX . "recurring r ON r.recurring_id = rd.recurring_id WHERE pr.product_id = " . (int)$product_id . " AND status = '1' AND pr.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' ORDER BY sort_order ASC");

                 
                $query = $this->db->query("SELECT ovd.name, po.product_id,ov.image as color_image FROM oc_product_option po JOIN oc_product_option_value pov ON (po.product_option_id = pov.product_option_id) 
JOIN oc_option_value ov  ON (ov.option_value_id = pov.option_value_id)
JOIN  (select * from oc_product_related where product_id='" . (int)$product_id . "') pr  ON ( po.product_id = pr.related_id) 
left JOIN oc_option_value_description ovd  ON (ov.option_value_id = ovd.option_value_id)
where  pov.option_id='13'  ");
