...
class ControllerNewsBlogCategory extends Controller {
	...
	public function index() {
		if (empty($this->session->data['check_version_newsblog'])){
			$this->session->data['check_version_newsblog'] = 
				@file_get_contents('http://nedorogoi-internet-magazin.ru/check_version.php?now='.$this->version_newsblog.'&version='.VERSION.'&site='.$_SERVER['HTTP_HOST']);
		}
	...
	protected function validateForm() {
		...
		$dir = opendir(DIR_CACHE);

		$string_to_search='cache.';

		while(($file = readdir($dir)) !== false) {
			$mass_sa = strstr($file,$string_to_search);
			if(trim($mass_sa) != "") {
				if (strlen($mass_sa) > 0){
					unlink(DIR_CACHE.$mass_sa);
				}
			}
		}
		closedir($dir);
