...
class ControllerToolMUpload extends Controller { 
  ...
  //    		      $multi_dir="catalog/";
				...
				if(!empty($this->request->get['product_id']) 
					&& $this->config->get('multiimageuploader_segment') == "product_id"){
					$multi_dir .= 'products/';
					$product_id = $this->request->get['product_id'] . '';
					$cpi_length = strlen($product_id);

					for($i = 0; $i < (6 - $cpi_length); $i++){
						$product_id = '0' . $product_id;
					}
					$multi_dir .= $product_id . '/';
				}
		        ...
		        $tmp_dir = '';
                foreach (explode('/', $multi_dir) as $value) {
                    $tmp_dir .= $value . '/';
					if (!is_dir ( DIR_IMAGE . $tmp_dir )) {
						mkdir( DIR_IMAGE . $tmp_dir, 0777);
					}  		               
                }
