        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-bestseller" class="form-horizontal">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              ...
              <div id="bestseller-product" class="well well-sm" style="height: 150px; overflow: auto;">
                <?php foreach ($products as $product) { ?>
                <div id="bestseller-product<?php echo $product['product_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $product['name']; ?>
...
$('input[name=\'product_name\']').autocomplete({
	...
	select: function(item) {
		...
		$('#bestseller-product' + item['value']).remove();
		
		$('#bestseller-product').append('<div id="bestseller-product' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="product[]" value="' + item['value'] + '" /></div>');	
	}
});
	
$('#bestseller-product').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
});
//--></script></div>
<?php echo $footer; ?>